...
function renderRecognition(recognition) {
  ...
  $("#recognition-hub").innerHTML = `<div class="recognition-balances">${["activity", "impact", "legacy"].map((lane) => `<article><span>${lane}</span><strong>${Number(balances[lane] || 0)}</strong><small>available</small></article>`).join("")}</div><div id="recognition-action" class="recognition-action muted">${activePerks.length ? `${activePerks.length} active perk${activePerks.length === 1 ? "" : "s"} this week.` : "Choose a perk to activate for this week."}</div>${breakdown.length ? `<section class="recognition-breakdown"><h3>Balance Breakdown</h3>${breakdown.map((item) => `<div class="recognition-breakdown-row"><span>${item.lane || "Lane"}</span><div><strong>${item.label || "Recognition points"}</strong><small>${item.detail || "Tracked from league activity."}</small></div><b>${Number(item.points || 0) > 0 ? "+" : ""}${Number(item.points || 0)}</b></div>`).join("")}</section>` : ""}<div class="recognition-columns"><section><h3>Recognition Leaders</h3>${leaders.map((leader) => `<div class="recognition-leader"><span>${leader.lane}</span><div><strong>${leader.name}</strong><small>${leader.detail || "League recognition"}</small></div><b>${leader.points}</b></div>`).join("") || `<p class="muted">Recognition leaders will appear after coach events are tracked.</p>`}</section><section><h3>Weekly Challenge</h3><article class="challenge-card"><strong>${challenge.title || "Clean Week Checklist"}</strong><p>${challenge.detail || "Schedule, communicate, stream, and finish on time."}</p><small>${challenge.bonus || "Bonuses will appear here."}</small></article>${activePerks.length ? `<div class="active-perks"><h3>Active Perks</h3>${activePerks.map((perk) => `<span>${perk.name} · ${perk.lane}</span>`).join("")}</div>` : ""}</section></div><div class="perk-grid">${perks.map((perk) => `<button class="perk-card ${perk.status === "locked" ? "locked" : ""} ${perk.status === "active" ? "active" : ""}" type="button" title="${perk.detail || ""}" ${perk.status === "available" ? `data-recognition-perk="${perk.id}"` : "disabled"}><span>${perk.lane}</span><strong>${perk.name}</strong><small>${perk.status === "locked" ? "Locked" : perk.status === "active" ? "Active" : `Cost ${perk.cost}`}</small></button>`).join("")}</div>`;
}
